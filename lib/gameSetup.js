...
exports.editGame = (game, type, actionObj, message) => {
  switch (type) {
    ...
    case "execute":
      const executeResult = executePrisoner(game.playersList, game.messagesHistory, game.startTime);
...
exports.processAnimationQueue = (game, io, roomId, rooms) => {
  // already paused or nothing to play
  ...
  io.to(roomId).emit("triggerAnimationForAll", animation.type);
  ...
  setTimeout(() => {
    ...
    // ▶️ play next animation if exists
