...
exports.handleWolvesVote = (playersList, messagesHistory) => {
  ...
  if (!mostVotedAgainstPlayer) {
    ...
    if (wasProtected) {
      playersList = reinitializeProtection(playersList, mostVotedAgainstPlayer);
    }

    const wasHealed = mostVotedAgainstPlayer.isHealed;
    if (wasHealed) {
      playersList = reinitializeHeal(playersList, mostVotedAgainstPlayer);
    }
    ...
    if (mostVotedAgainstPlayer.role.name === "Serial Killer") {
      messagesHistory.unshift({
        time: getCurrentTime(),
        author: "",
        msg: `{serverContent.wolfVote.outcome.null} DEV -- unableToKillSK`,
      });
    } else if (wasHealed) {
      messagesHistory.unshift({
        time: getCurrentTime(),
        author: "",
        msg: `{serverContent.wolfVote.outcome.wasHealed} DEV -- healedByDoctor`,
      });
    } else if (wasProtected) {
      messagesHistory.unshift({
        time: getCurrentTime(),
        author: "",
        msg: `{serverContent.wolfVote.outcome.null} DEV -- protectedByWitch`,
      });
      ...
      messagesHistory.unshift({
        time: getCurrentTime(),
        author: "",
        msg: `{serverContent.wolfVote.outcome.null} DEV -- bittenByWolves`,
      });
    } else {
      playersList = killSelectedPlayer(mostVotedAgainstPlayer.id, playersList);
      messagesHistory.unshift({
