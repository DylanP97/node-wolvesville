...
exports.handleWolvesVote = (playersList, messagesHistory) => {
    ...
    if (!mostVotedAgainstPlayer) {
        ...
        if (mostVotedAgainstPlayer.role.name === "Serial Killer") {
            messagesHistory.unshift({ time: getCurrentTime(), author: "", msg: `The wolves couldn't kill their target (serial killer)!` });
        } else if (wasHealed) {
            messagesHistory.unshift({ time: getCurrentTime(), author: "", msg: `The wolves couldn't kill their target! (healed by the doctor)` });
        } else {
            playersList = killSelectedPlayer(mostVotedAgainstPlayer.id, playersList);
