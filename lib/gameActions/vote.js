...
exports.handleWolvesVote = (playersList, messagesHistory) => {
  ...
  if (!mostVotedAgainstPlayer) {
    ...
    if (wasProtected) reinitializeProtection(playersList, mostVotedAgainstPlayer);

    const wasHealed = mostVotedAgainstPlayer.isHealed;
    if (wasHealed) reinitializeHeal(playersList, mostVotedAgainstPlayer);
