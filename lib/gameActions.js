...
exports.handleVote = (playersList, messagesHistory, winningTeam) => {
  ...
  if (!mostVotedAgainstPlayer) {
    messagesHistory.push({ time: getCurrentDateTime(), author: "", msg: `The town couldn't decide who to kill!` });
  } else {
    playersList = this.killSelectedPlayer(mostVotedAgainstPlayer.id, playersList);
    if (mostVotedAgainstPlayer.role.name === "Fool") {
      messagesHistory.push({ time: getCurrentDateTime(), author: "", msg: `The fool won 🤡!` });
      winningTeam = {
        name: "fool",
        image: "https://res.cloudinary.com/dnhq4fcyp/image/upload/v1706531396/roles/fool_ngedk0.png",
      };
    } else {
      messagesHistory.push({ time: getCurrentDateTime(), author: "", msg: `The town decided to kill ${playersList[mostVotedAgainstPlayer.id].name} has a result of the vote! 💀` });
...
exports.handleWolvesVote = (playersList, messagesHistory, winningTeam) => {
  const mostVotedAgainstPlayer = this.findPlayerWithMostWolvesVotes(playersList);
  if (!mostVotedAgainstPlayer) {
    messagesHistory.push({ time: getCurrentDateTime(), author: "", msg: `No one was murdered by the wolves!` });
  } else {
    playersList = this.killSelectedPlayer(mostVotedAgainstPlayer.id, playersList);
    if (mostVotedAgainstPlayer.role.name === "Fool") {
      messagesHistory.push({ time: getCurrentDateTime(), author: "", msg: `The fool won 🤡!` });
      ...
      messagesHistory.push({ time: getCurrentDateTime(), author: "", msg: `The wolves murdered ${playersList[mostVotedAgainstPlayer.id].name} has a result of the vote! 💀` });
      // checkIfIsInLove(mostVotedAgainstPlayer, updatedPlayersList, setUpdatedPlayersList, displayAction);
    }
  }
...
exports.murder = (newPlayersList, newMessagesHistory, action) => {
  const attackedPlayer = newPlayersList.find((ply) => ply.id == action.selectedPlayerId)

  const wasHealed = attackedPlayer.isHealed

  if (wasHealed) {
    newMessagesHistory.push({ time: getCurrentDateTime(), author: "", msg: `Someone wounds were healed by the doctor last night! 👩‍⚕️` })
    console.log("Someone wounds were healed by the doctor last night!")
    newPlayersList = newPlayersList.map((player) => {
    ...
    newPlayersList = newPlayersList.map((ply) => {
      if (ply.id === action.selectedPlayerId) {
        newMessagesHistory.push({ time: getCurrentDateTime(), author: "", msg: `${ply.name} was killed last night by the serial killer. 💀🔪` })
        console.log(`${ply.name} was killed last night by the serial killer.`)
        return {
          ...ply,
          isAlive: false,
        }
  ...
  const newPlayersListEdited = newPlayersList
  const newMessagesHistoryEdited = newMessagesHistory

  // need to add inlove outcome
  return { newPlayersListEdited, newMessagesHistoryEdited }
}
...
exports.heal = (action, playersList) => {
  return playersList.map((player) => {
    if (player.id === action.selectedPlayerId) {
      return {
        ...player,
