...
exports.handleVote = (playersList, messagesHistory, winningTeam) => {
  ...
  if (!mostVotedAgainstPlayer) {
    messagesHistory.push({ author: "", msg: `The town couldn't decide who to kill!` });
  } else {
    playersList = this.killSelectedPlayer(mostVotedAgainstPlayer.id, playersList);
    if (mostVotedAgainstPlayer.role.name === "Fool") {
      messagesHistory.push({ author: "", msg: `The fool won 🤡!` });
      winningTeam = {
        name: "fool",
        image: "https://res.cloudinary.com/dnhq4fcyp/image/upload/v1706531396/roles/fool_ngedk0.png",
      };
    } else {
      messagesHistory.push({ author: "", msg: `The town decided to kill ${playersList[mostVotedAgainstPlayer.id].name} has a result of the vote!` });
...
exports.handleWolvesVote = (playersList, messagesHistory, winningTeam) => {
  ...
  if (!mostVotedAgainstPlayer) {
    messagesHistory.push({ author: "", msg: `No one was murdered by the wolves!` });
  } else {
    playersList = this.killSelectedPlayer(mostVotedAgainstPlayer.id, playersList);
    if (mostVotedAgainstPlayer.role.name === "Fool") {
      messagesHistory.push({ author: "", msg: `The fool won 🤡!` });
      ...
      messagesHistory.push({ author: "", msg: `The wolves murdered ${playersList[mostVotedAgainstPlayer.id].name} has a result of the vote!` });
      // checkIfIsInLove(mostVotedAgainstPlayer, updatedPlayersList, setUpdatedPlayersList, displayAction);
    }
  }
...
exports.murder = (newPlayersList, newMessagesHistory, action) => {
  ...
  if (wasHealed) {
    newMessagesHistory.push({ author: "", msg: `Someone wounds were healed by the doctor last night!` })
    ...
    newPlayersList = newPlayersList.map((ply) => {
      if (ply.id === action.selectedPlayerId) {
        newMessagesHistory.push({ author: "", msg: `${ply.name} was killed last night by the serial killer.` })
