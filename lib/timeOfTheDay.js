...
exports.toDayTime = (game) => {
    ...
    if (game.aliveList.length > 1) {
        const { playersListEdit, messagesHistoryEdit, winningTeamEdit } = handleWolvesVote(playersList, messagesHistory, winningTeam)

        playersList = playersListEdit;
        messagesHistory = messagesHistoryEdit
        winningTeam = winningTeamEdit
    }

    playersList = releasePrisoners(playersList);
    console.log("did releasePrisoners function returned a correct playersList?")
