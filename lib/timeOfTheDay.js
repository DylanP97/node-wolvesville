...
exports.toNightTimeAftermath = (game) => {
  ...
  let skAnimationTriggered = false; // Track if SK animation should trigger
  ...
  game.registeredActions.forEach((action) => {
    if (action.type === "murder") {
      const { playersListEdit, messagesHistoryEdit, killedBySKEdit, shouldTriggerAnimation } = murder(
        playersList,
        messagesHistory,
        killedBySK,
        action
      ...
      // Set flag if animation should trigger
      if (shouldTriggerAnimation) {
        skAnimationTriggered = true;
      }
  ...
  // whathever happens it gets reinitialized
  game.playersList = playersList
  game.playersList = reinitializeHeal(game.playersList);
  ...
  // Store animation flag in game object
  game.skAnimationTriggered = skAnimationTriggered;
