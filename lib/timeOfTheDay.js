...
exports.toNightTimeAftermath = (game) => {
  ...
  if (game.aliveList.length > 1) {
    ...
    // Track role action stats
    if (game.roleActions) {
      game.roleActions.wolfKills += wolfKills;
      game.roleActions.doctorSaves += doctorSaves;
      game.roleActions.witchSaves += witchSaves;
    }
  }
  ...
  // Track SK kills (difference from previous count)
  const newSKKills = killedBySK - (game.killedBySK || 0);
  if (game.roleActions && newSKKills > 0) {
    game.roleActions.skKills += newSKKills;
  }
